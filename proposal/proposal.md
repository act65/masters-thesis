...
Thus, to _efficiently_ scale artificial intelligence to the real world and all of its complexity (high dimensionality, non-linearity and non-locality), it will be necessary for knowledge to be transferred between domains.

## Transfer as decomposition

One way to achieve transfer learning is to decompose complex observations into a set of modules/'atomic'-factors. Or in other words, to disentangle independent factors. For example we could take a symmetry group, and rewrite it as the composition of a single element with only a few transformations. Thus we have decomposed the group into is parts.
...
By definition, TRL requires the testing of learners on many different tasks. But this can take thousands of CPU/GPU hours to evaluate... (worse than other types of machince learning do to the use of data hunger function approimators, neural networks, and the sparsity of the supervision). I think there is a need for cleverly designed benchmarks that don't require thousands of dollars to bench. Currently TRL is a game for players with resources, which means that only Google and Google-Deepmind get to play.
...
Finally, the real world is just a lot more complex than these games we test our algorithms on. If we want to scale to the real world, we need to make our algorithms more data efficient. Transfer learning is a solution.
...
4. When learning, we want to know if existing knowledge is useful for a new task. How can we assign credit to a given 'module' of knowledge? Is 'counterfactual' credit assignment necessary or sufficient?
