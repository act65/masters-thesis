...
Existing solvers of MDPs require solving a non-convex optimisation problem (?!? ref).
It does have some nice properties, like having a unique optima under the bellman
...
% Linearity in MDPs
% Polytope. LPs
% !!!?!? (if the bellman eqn is non linear / non convex then why can we solve it with LP?!)
...
It turns out that linearising a MDP is not a new idea. \cite{Todorov2006, Wang, Pires2016, Levine2019}
...
% intuition
To build a LMDP that acts similarly to a MDP Todorov et al. \cite{Todorov2006} use a few main tricks;
...
However, while this make sense intuitively, it turn out to be wrong. (more details here...)
...
\subsubsection{Decoding the optimal LMDP policy}
