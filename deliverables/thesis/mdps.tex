...
{\color{red} want refs for each of these!?}
...
A general intuition for what is going on when solving a MDP is; actions are taken
sequentially (eg.) and
need to be conditioned on observations (eg.).
...
as a tuple, \(\{\mathcal S, \mathcal A, P(s_{t+1} \mid s_t, a_t),R(s_t, a_t, s_{t+1}), \gamma\}\).
...
and finally, \(r(s_t, a_t, s_{t+1})\) is the reward function, \textit{whether you won (+1) or lost (-1) the game }.
...
You can find more examples in \ref{MDP-examples}.
...
\footnotetext{Note that the ability to solve the Bellman equation analytically does not allow
