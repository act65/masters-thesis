...
{\color{red}example. If I tell you that X and Y are the same, how can you use that to do Z?}
...
{\color{red}Is the set of classes complete?}
...
There exist some interesting question about the difference in performance between these abstractions.
Such as the advantages \textit{State and action abstraction} versus \textit{State-action abstraction},
or the advantages of \textit{goal-like} versus \textit{option-like} temporal abstraction.
...
\paragraph{Action abstraction} groups together actions that are similar. For
...
To build an abstraction, you need some notion of how two objects can be similar. {\color{red}(ref?!}
...
\\
\chi(x, x') &= \int_{\pi \in \subset \Pi} D(c(x, \pi, \gamma), c(x, \pi, \gamma))
\end{align*}
...
Current formulation doesnt work if we want to compare trajectories, rather than expected cumulants...
How powerful is the family of similarity measures that use expected cumulants versus the family that uses trajectories?!
...
\begin{itemize}
  \tightlist
  \item Transition abstraction. Set $f(\zeta_{t+k+1}) = s_{t+k+1}$ then the cumulant becomes the discounted state visitation count (if we have encoded the states as onehots - so only works for finite state spaces...).
  Two $x$s with the same expected discounted state visitation count for all policies must have the same transitions!?!??!?!
  \item Optimal policy. Shit. $f(\zeta_{t+k+1}) = s_{t+k+1}$ then the cumulant becomes the return. How do get the Q fn?!?!
\end{itemize}
