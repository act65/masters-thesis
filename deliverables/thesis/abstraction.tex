...
% example from word2vec. perserves semantic relationships.
...
These definitions might be different from each other. For example on space might be continuous and the other discrete.}
...
\subsection{Exploiting abstractions}
...
\footnotetext[26]{This argument needs to be formalised. But, related work has shown that if the value of the optimal action is not preserved then
applying the Bellman equation no longer guarantees that optimal policies are also optimal in the abstraction. \cite{Jong2005}
}
