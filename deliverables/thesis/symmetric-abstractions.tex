...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State $t=0$
		\While{not converged}
		\State $(s, a, r, s', a')$ \Comment{Observe}
		\State $\tau, R \sim P_{\theta_t}(\cdot | D)$ \Comment{Sample a model}
		\State $Q_{t+1}(s, a) = R(s, a) + \gamma \tau(s'| s, a) Q_t(s', a')$ \Comment{Bellman operator}
		\State $\theta_{t+1} = \theta_t - \eta \nabla_{\theta} P((s, a, r, s', a')| \theta_t)$ \Comment{Max a posteriori}
		\State $t += 1$
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State $(s, a, r, s', a')$ \Comment{Observe}
		\State $\tau, R \sim P_{\theta_t}(\cdot | D)$ \Comment{Sample a model}
		\State $S \sim P(\cdot | D)$ \Comment{Sample a state abstraction}
		\State $\tilde \tau, \tilde r, \tilde Q_t = S(\tau, r, Q_t)$ \Comment{Do a model reduction}
...
Where we estimate $P(A|D)$ as ... .
$A \in R^{n\times n}$.

\subsubsection{Biased Abstracted Thompson sampling}

The only difference between \texttt{Abstracted Thompson Sampling} and \texttt{Biased Abstracted Thompson Sampling}
is how we construct the posteriori over abstractions, $P(A|D)$.

We have a prior belief that symmetric $\tau, r$ should be more likely, this is captured by the distribution $P(A)$.
MAP estimate of the abstraction is $P(A | D) = \frac{P(D | A)P_S(A)}{P(D)}$.

\paragraph{Rejection sampling}

\begin{displayquote}
\textsl{But, how do we actually construct $P_S(A)$?}
\end{displayquote}
