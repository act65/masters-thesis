...
\begin{align*}
\mu(M) &:= \mathop{\mathbb E}_{\pi\sim\Pi}\Big[V_M^{\pi} \Big]\\
...
\textbf{Open problem:} If an MDP has sub-optimality,  $\epsilon^{* }(M)$,
...
This is an inference problem, for which we hope to apply control later on.
This is also known as model based RL.

We could search through models using supervision from next step prediction.
This is a common approach to model-based RL.
